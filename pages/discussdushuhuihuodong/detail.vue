...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussdushuhuihuodongCleanType')
			if(cleanType){
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussdushuhuihuodong', this.id);
		...
		methods: {
			...
			async init(){
				...
				let res = await this.$api.info('discussdushuhuihuodong', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussdushuhuihuodong', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussdushuhuihuodong/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
